...
@Autonomous(name="auto blue (right)", group="TestBot") //Aarav Jagtiani, Rishabh Mahesh
// simple auto
// robot senses barcode, drops preload element in correct level, spins duck off carousel, parks in storage unit
public class Auto_Blue_Right extends OpMode {
    ...
    private enum State {
        ...
        STRAFE_TO_ALIGN_WITH_STORAGE_UNIT,//robot strafes to align with storage unit
        BACK_INTO_STORAGE //robot backs into storage unit to make sure
    ...
    public void loop(){
        switch (currentState){
            ...
            case SPIN_CAROUSEL:
                ...
                newState(State.STRAFE_TO_ALIGN_WITH_STORAGE_UNIT);
                break;
            case STRAFE_TO_ALIGN_WITH_STORAGE_UNIT:

                //reset encoders
                robot.reset_encoder();

                //strafe for 570 positions
                while(robot.rightFront.getCurrentPosition()<904) {
                    robot.strafe_right(0.3);
                }

                newState(State.BACK_INTO_STORAGE);
                break;
